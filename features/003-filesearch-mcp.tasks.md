...
- **Acceptance Criteria**:
  - [x] `npm install node-adodb` succeeds
  - [x] POC script connects using `Provider=Search.CollatorDSO;Extended Properties='Application=Windows'`
  - [x] POC executes `SELECT TOP 10 System.ItemPathDisplay, System.FileName, System.Size, System.DateModified FROM SystemIndex` and returns results
  - [x] Result rows can be parsed: path (string), filename (string), size (number), date (Date)
  - [x] Document multi-valued property behavior (System.Author, System.Keywords): how node-adodb returns arrays vs delimited strings
  - [x] Document error behavior when WSearch service is stopped
  - [x] CONTAINS and FREETEXT queries work: `WHERE CONTAINS(*, '"test"')` and `WHERE FREETEXT(*, 'test document')`
  - [x] SCOPE clause works: `WHERE SCOPE='file:C:/Users'`
  - [x] Results documented in `src/services/filesearch/README.md`
- **Test Requirements**:
  - [x] Manual testing on Windows with active Search Indexer
  - [x] Document data types returned by node-adodb for each column
  - [x] Verify behavior with non-indexed paths
- **Effort**: M (2-3 days)
- **Dependencies**: None
...
- **Acceptance Criteria**:
  - [x] Directory created: `src/services/filesearch/`
  - [x] Directory created: `src/services/filesearch/__tests__/`
  - [x] File created: `src/services/filesearch/types.ts` with all interfaces:
  ...
  - [x] File created: `src/services/filesearch/config.ts`:
...
- **Acceptance Criteria**:
  - [x] File created: `src/services/filesearch/scope-validator.ts`
  - [x] `ScopeValidator` class with methods:
  ...
  - [x] When allowedPaths is empty, all local paths are allowed (MVP behavior)
- **Test Requirements**:
  - [x] `src/services/filesearch/__tests__/scope-validator.test.ts`: 20+ tests
...
- **Acceptance Criteria**:
  - [x] File created: `src/services/filesearch/query-builder.ts`
  - [x] `buildSearchQuery(params: FileSearchQueryParams): BuiltQuery` function:
  ...
  - [x] `countSql` generates matching SELECT COUNT(*) query
  - [x] SQL injection prevention functions:
  ...
  - [x] All parameters combined with AND logic
  - [x] Generates valid SQL when no filters provided (returns recent files)
  - [x] Generates valid SQL with any single parameter
  - [x] Generates valid SQL with all parameters combined
- **Test Requirements**:
  - [x] `src/services/filesearch/__tests__/query-builder.test.ts`: 50+ tests
  ...
  - [x] All tests pass, 100% coverage on sanitization functions
- **Effort**: L (3-4 days)
...
- **Acceptance Criteria**:
  - [x] File created: `src/services/filesearch/oledb-executor.ts`
  - [x] `OleDbExecutor` class:
  ...
  - [x] `OleDbResult` interface: `{ rows: Record<string, unknown>[]; success: boolean; errorMessage?: string; executionTimeMs: number }`
  ...
  - [x] Connection string: `Provider=Search.CollatorDSO;Extended Properties='Application=Windows'`
  - [x] Connection per query (no pooling for MVP)
  - [x] Proper cleanup on errors (connection closed in finally block)
- **Test Requirements**:
  - [x] `src/services/filesearch/__tests__/oledb-executor.test.ts`: 20+ tests
  ...
  - [x] All tests pass with mocked node-adodb
- **Effort**: M (2-3 days)
...
- **Acceptance Criteria**:
  - [x] File created: `src/services/filesearch/result-mapper.ts`
  - [x] `mapOleDbRow(row: Record<string, unknown>): FileSearchEntry` function:
  ...
  - [x] `mapOleDbRows(rows: Record<string, unknown>[]): FileSearchEntry[]` — batch mapper
  - [x] Graceful handling of null/undefined/missing fields for all columns
  - [x] Multi-valued property handling:
...
- **Test Requirements**:
  - [x] `src/services/filesearch/__tests__/result-mapper.test.ts`: 25+ tests
...
- **Acceptance Criteria**:
  - [x] File created: `src/services/filesearch/path-anonymizer.ts`
  - [x] `PathAnonymizer` class:
  ...
  - [x] Author anonymization: full name → `[ANON_USER_N]`
  - [x] Consistency: same username in path and same author name map to same anon ID (via shared PiiAnonymizer)
  - [x] Non-user paths (e.g., `C:\Program Files\...`) pass through unchanged
- **Test Requirements**:
  - [x] `src/services/filesearch/__tests__/path-anonymizer.test.ts`: 25+ tests
...
- **Acceptance Criteria**:
  - [x] File created: `src/services/filesearch/provider.ts`
  - [x] `FileSearchProvider` class:
  ...
  - [x] Exception types: `PermissionDeniedException`, `ValidationException`, `OperationFailedException`, `ScopeViolationException`
  - [x] Metrics tracked: queries executed, queries failed, results returned, execution time
  - [x] Graceful degradation when WSearch unavailable
- **Test Requirements**:
  - [x] `src/services/filesearch/__tests__/provider.test.ts`: 30+ tests
    ...
    - Timeout handling
  - [x] All tests pass with mocked OLE DB executor, >80% coverage
- **Effort**: L (2-3 days)
...
- **Acceptance Criteria**:
  - [x] `src/graphql/schema.ts` updated with:
    - `SearchMode` enum (CONTAINS, FREETEXT)
  ...
  - [x] Reuses existing `PageInfo` type from EventLog
  - [x] Schema compiles and Apollo Server starts without errors
  - [x] Introspection shows new types and query
- **Test Requirements**:
  - [x] Existing schema tests still pass
  - [x] New schema types can be queried via introspection
...
- **Acceptance Criteria**:
  - [x] File created: `src/graphql/filesearch.resolver.ts`
  - [x] `FileSearchErrorCode` enum with all error codes:
  ...
  - [x] `FileSearchGraphQLError` class (extends GraphQLError, includes code + timestamp)
  - [x] `fileSearchResolver` function:
  ...
  - [x] `src/graphql/resolvers.ts` updated to spread `filesearchResolver.Query`
  - [x] Resolver context extended with `filesearchProvider` and `filesearchAnonymizer`
- **Test Requirements**:
  - [x] `src/graphql/__tests__/filesearch.resolver.test.ts`: 35+ tests
  ...
  - [x] All tests pass with mocked provider, >80% coverage
- **Effort**: L (2-3 days)
...
- **Acceptance Criteria**:
  ...
  - [x] Health check includes FileSearch service status
  - [x] Server starts without errors when WSearch is available
  - [x] Server starts with warning when WSearch is unavailable (graceful degradation)
- **Test Requirements**:
  - [x] Server starts and responds to health check
  - [x] `npm run build` succeeds
  - [x] Existing EventLog functionality unaffected
- **Effort**: S (0.5 days)
...
- **Acceptance Criteria**:
  - [x] File created: `src/services/filesearch/mcp-service.ts`
  ...
  - [x] File created: `src/mcp/filesearch-service.ts`
  ...
  - [x] `src/mcp/index.ts` updated to register FileSearchMcpService
- **Test Requirements**:
  - [x] `src/services/filesearch/__tests__/mcp-service.test.ts`: 15+ tests
  ...
  - [x] `src/mcp/__tests__/filesearch-service.test.ts`: 15+ tests
...
- **Acceptance Criteria**:
  - [x] Integration test: GraphQL query → resolver → provider → mocked OLE DB → response
  - [x] Integration test: MCP tool call → service → GraphQL → resolver → mocked provider → response
  - [x] Pagination integration: multiple pages with correct pageInfo
  - [x] Filter combination integration: multiple filters applied correctly
  - [x] Error propagation: OLE DB errors surface correctly through all layers
  - [x] PII anonymization: verify no raw PII in GraphQL responses
  - [x] Metrics: verify metrics update across multiple queries
  - [x] All existing tests still pass (no regressions)
- **Test Requirements**:
  - [x] `src/services/filesearch/__tests__/integration.test.ts`: 15+ tests
  - [x] `npm test` — all tests pass including existing 313+ tests
- **Effort**: M (1-2 days)
...
- **Acceptance Criteria**:
  - [x] File created: `src/services/filesearch/__tests__/security.test.ts`
  - [x] Tests cover all injection vectors:
    ...
    - Comment injection (`--`, `/* */`)
  - [x] All vectors confirmed to produce safe SQL (no injection possible)
  - [x] Each test documents the attack vector and expected safe output
- **Test Requirements**:
  - [x] 30+ security-focused tests
  - [x] 100% pass rate
  - [x] Document each attack vector in test comments
- **Effort**: M (1-2 days)
...
- **Acceptance Criteria**:
  - [x] Tests verify SCOPE clause is always applied when allowedPaths configured
  - [x] Tests verify path parameter outside scope is rejected before query execution
  - [x] Tests verify post-query result filtering removes any results outside scope (defense in depth)
  - [x] Tests verify path normalization prevents bypass:
    - `C:\Users\..\Windows\System32` → rejected
    - Mixed slashes don't bypass
    - Case variations don't bypass
    - URL-encoded paths don't bypass
  - [x] Tests verify UNC paths are rejected
- **Test Requirements**:
  - [x] Add to `src/services/filesearch/__tests__/security.test.ts`: 15+ scope tests
  - [x] All pass
- **Effort**: S (1 day)
...
- **Acceptance Criteria**:
  - [x] Tests verify all file paths containing user profiles are anonymized in responses
  - [x] Tests verify all author fields are anonymized in responses
  - [x] Tests verify PII does not appear in log messages (mock logger, inspect calls)
  - [x] Tests verify PII does not appear in error messages returned to GraphQL clients
  - [x] Tests verify PII does not appear in metrics or audit data
  - [x] Tests verify consistency: same user gets same anon ID across multiple queries
  - [x] Tests verify shared store: same user gets same anon ID across EventLog and FileSearch
- **Test Requirements**:
  - [x] Add to `src/services/filesearch/__tests__/security.test.ts`: 15+ PII tests
  - [x] All pass
- **Effort**: S (1 day)
...
- **Acceptance Criteria**:
  - [x] Windows Search service not running → clear diagnostic error
  - [x] OLE DB provider not installed → clear error with instructions
  - [x] Query timeout → timeout error with duration
  - [x] Empty result set → valid empty response (not error)
  - [x] Maximum offset reached (>10000) → validation error
  - [x] All filter parameters at extreme values (max int, empty strings, very long strings)
  - [x] Concurrent queries → no race conditions
  - [x] Service disabled after query starts → graceful handling
  - [x] Invalid date strings → clear validation error
  - [x] node-adodb process crash → caught and wrapped
- **Test Requirements**:
  - [x] `src/services/filesearch/__tests__/edge-cases.test.ts`: 20+ tests
  - [x] All pass
- **Effort**: M (1-2 days)
...
- **Acceptance Criteria**:
  - [x] Validation matrix documented and tested:
    ...
    | fileType | .ext format | ✓ | ✓ |
  - [x] Both layers reject invalid input independently (defense in depth)
  - [x] Error messages are clear and don't leak system information
- **Test Requirements**:
  - [x] Validation tests added across resolver and provider test files
  - [x] All pass
- **Effort**: S (0.5 days)
...
- **Acceptance Criteria**:
  - [x] `src/services/filesearch/API.md` created:
  ...
  - [x] `docs/TOOLS.md` updated with FileSearch tool definitions:
    - `filesearch_query` parameters and examples
    - `filesearch_status` parameters and examples
  - [x] `src/services/filesearch/README.md` updated with:
...
- **Acceptance Criteria**:
  - [x] All public classes and methods have JSDoc with @param, @returns, @throws, @example
  - [x] Security-critical code has `// SECURITY:` comments
  - [x] All files have module-level documentation headers
  - [x] No TODO or FIXME comments remain (except for documented future work)
  - [x] All files < 300 lines
  - [x] Code follows existing project conventions (naming, structure, patterns)
...
- **Acceptance Criteria**:
  - [x] `npm run test:coverage` shows >80% line and branch coverage for all FileSearch files
  - [x] All 313+ existing tests still pass (no regressions)
  - [x] New FileSearch tests: 250+ tests total across all test files
  ...
  - [x] Zero flaky tests
  - [x] Performance: all tests complete in < 30 seconds
- **Effort**: M (1 day)
- **Dependencies**: All Phase 1-3 tasks
...
- **Acceptance Criteria**:
  - [x] Verify Windows Search service is running and indexing
  - [x] Execute fileSearch GraphQL query and receive real results
  - [x] Verify all metadata fields populated from real index (path, name, size, dates, author, title, tags)
  - [x] Verify CONTAINS search returns relevant results
  - [x] Verify FREETEXT search returns relevant results
  - [x] Verify file type filtering works
  - [x] Verify date range filtering works
  - [x] Verify size range filtering works
  - [x] Verify path scoping works
  - [x] Verify pagination across real result sets
  - [x] Verify PII anonymization on real file paths
  - [x] Document any discrepancies between mock tests and real behavior
- **Effort**: M (1-2 days)
...
- **Acceptance Criteria**:
  - [x] Configure Claude with FileSearch MCP tools
  - [x] `filesearch_query` tool discoverable in tools/list
  - [x] `filesearch_status` tool discoverable
  - [x] Claude can search for files by name
  - [x] Claude can search by content keywords
  - [x] Claude can filter by file type, size, dates
  - [x] Claude can paginate through results
  - [x] Results display correctly with anonymized paths
  - [x] Error messages are helpful when service unavailable
- **Effort**: S (0.5 days)
...
- **Acceptance Criteria**:
  - [x] Simple query (filename search): <200ms
  - [x] Complex query (text search + filters): <400ms
  - [x] Pagination (offset 0-500): <200ms
  - [x] Pagination (offset 5000): <500ms (acceptable degradation)
  - [x] COUNT(*) query: <200ms
  - [x] Memory usage during query: <500MB
  - [x] 10 concurrent queries: all complete without errors
  - [x] Document actual performance numbers
